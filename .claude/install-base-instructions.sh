#!/bin/bash
# Installs the portable base agent instructions into a repo.
# Usage:  bash install-base-instructions.sh /path/to/FMBL-wristbands
set -euo pipefail
REPO="${1:?usage: install-base-instructions.sh <repo-path>}"
cd "$REPO"
git rev-parse --git-dir >/dev/null   # bail if not a repo
BR="claude/base-agent-instructions"
git checkout -b "$BR" 2>/dev/null || git checkout "$BR"
mkdir -p .claude
cat > .claude/BASE-INSTRUCTIONS.md <<'BASEEOF'
...
IMPORT='@.claude/BASE-INSTRUCTIONS.md'
HDR='## 0. Base Agent Instructions — READ FIRST'

if [ -f CLAUDE.md ]; then
  if grep -q 'BASE-INSTRUCTIONS' CLAUDE.md; then
    echo "CLAUDE.md already imports it — leaving alone."
  else
    # insert after the first heading if there is one, else prepend
    { if head -1 CLAUDE.md | grep -q '^# '; then
        head -1 CLAUDE.md; echo; echo "$HDR"; echo; echo "$IMPORT"; echo
        tail -n +2 CLAUDE.md
      else
        echo "$HDR"; echo; echo "$IMPORT"; echo; cat CLAUDE.md
      fi
    } > CLAUDE.md.new && mv CLAUDE.md.new CLAUDE.md
    echo "Wired import into existing CLAUDE.md."
  fi
else
  printf '# %s — Claude Code Context\n\n%s\n\n%s\n' "$(basename "$PWD")" "$HDR" "$IMPORT" > CLAUDE.md
  echo "Created CLAUDE.md."
fi

# pointer line in any subagent definitions
PTR='**Base instructions:** read `.claude/BASE-INSTRUCTIONS.md` before acting — it is binding on this agent.'
for f in .claude/agents/*.md; do
  [ -e "$f" ] || continue
  grep -q 'BASE-INSTRUCTIONS' "$f" && continue
  python3 - "$f" "$PTR" <<'PY'
import sys
p, ptr = sys.argv[1], sys.argv[2]
s = open(p).read()
parts = s.split('---\n', 2)
if len(parts) == 3:
    open(p,'w').write('---\n' + parts[1] + '---\n\n' + ptr + '\n\n' + parts[2].lstrip('\n'))
...
git add .claude/BASE-INSTRUCTIONS.md CLAUDE.md .claude/agents 2>/dev/null || git add -A
git commit -q -m "Add base agent instructions doctrine"
echo
echo "Committed on $BR. Review with: git show --stat"
echo "Then: git push -u origin $BR"
