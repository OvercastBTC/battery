...
- **E6 — TODAY/Home landing + Daily Readiness score. ✅ DONE** (audited by Lane D, 2026-08-31, against `master` `9cfb853`/`.104`). `#view-today` present (4 refs) as the fused readiness view described below; `today-view.test.mjs` exists and runs in the gate (`CLAUDE.md §5`'s 24-suite list). ~~No cross-tab home exists; the scoreboard is buried in GAME.~~ Promote a `#view-today` host `<section>` (make it the boot default) that fuses what the host already computes — `_batCounts` (arm/drills/body done), `_batFuel` (water/protein vs target), `pitchRestInfo()` (Pitch-Smart rest), day-type from `buildOutlook()` — into a single **0–100 Daily Readiness ring** + the existing tappable scoreboard rows. **Exclude the protein/supplement contribution whenever `profileTier(profile)==='youth'`** — the youth ring must never show macros. Anchors from the original analysis: GAME section ~7985, `renderScoreboard` ~8811, tabbar ~8025, `restoreLastView` ~8863, default tab ~8169 (line numbers now stale — file has grown well past 15k lines).
- **E7 — deep-link to nested tabs + tab dots + nav bugfixes. ✅ DONE** (audited by Lane D, 2026-08-31). `bat-nav` present (6 refs), `bat-poll` present (8 refs) — the host↔iframe postMessage seam described below shipped. **NAV-5 confirmed fixed:** no `.replace(/^./,c=>c)` no-op anywhere in `index.html`; `#youth-banner`'s default text is now neutral ("Youth mode — growth-based food protein + water, tuned for an active kid...") not the stale "Kole (6, active)" this entry used to describe. ~~Add a host→iframe `bat-nav` postMessage~~ (shipped; see §D for the shape). Light a `todo` dot on FUEL when behind on water (and adult-only protein), and a `warn` dot on GAME when Arm Guardian is RESTING. NAV-4 (GAME scoreboard stale on first visit) fixed via `bat-poll` broadcast on entry.
- **E8 — one-tap "Flow mode". ✅ DONE** (audited by Lane D, 2026-08-31). `startFlow` present (3 refs), `flowStep` present (8 refs); `flow-mode.test.mjs` asserts real behavior in the gate. Guided walkthrough of today's unchecked must-dos using E7 deep-links; youth variant = food/water/light-arm only.
- **E9 host-side — mixed, audited by Lane D 2026-08-31 (grepped `master` `9cfb853`, not guessed from this entry's text):**
  - arm-iframe native-dialog→modal conversion — **✅ DONE.** `uiConfirm()`/`uiAlert()` (ARM ~5444, FUEL ~9744) bridge to the host's `batteryModal()`/`modalConfirm()`/`modalAlert()` (~13757–13787) whenever embedded; the 4 remaining native `confirm()`/`alert()` calls are that bridge's documented "native fallback if parent unreachable" path, not leftover unconverted dialogs.
  - weekly report card — **✅ DONE.** `#week-card`/`week-body` present — shipped back in the `.55`/`.56` release per this file's own 2026-06-22 entry above (§ "NEW 2026-06-22").
  - Arm Guardian v2 — **✅ DONE.** Extensive rest-forecast/Pitch-Smart cross-check system live: `pitchRestInfo()`, GAME-tab Arm Guardian, the US-2.6 "REAL recovery status" divergence-stopper cross-checking the Rest chip against the actual required-rest countdown.
  - verify the Pitch Smart table against pitchsmart.org — **✅ DONE.** Inline comment at ~2607 marks the youth throwing-progression table "VERIFIED content only (Pitch Smart / AAP / Little League / Jaeger)".
  - game-day local notifications — **⬜ OPEN, genuinely missing.** Zero hits for `Notification(`/`notification.permission`/`requestPermission` anywhere in `index.html`. Not built.
  - consistency heatmap — **✅ DONE.** `consistencyMap()` (~14611) exists, explicitly labeled "Training Consistency heatmap in DATA (host E9)" in its own comment; `consistency` test suite runs in the gate.
